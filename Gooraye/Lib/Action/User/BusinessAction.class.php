...
class BusinessAction extends UserAction{
    ...
    /**
     * @param $module 模块名称,必须大写开头
     * @param $type  行业类型,必须小写
     * @param $post  POST 提交数据 (del)
     * @param $return_func 返回方法
     * @author D.J. www.gooraye.com
     *
     * */
    public function fitness_add($module,$type,$return_func){
        //$module = ucfirst(strtolower($module)); //可以删除
        $Photo = M("Photo");
        ...
        if(IS_POST){
            ...
            if(!$check){
                exit($this->error('包含敏感字符,或者是不允许字串!',U("$module/$return_func",array('token'=>session('token'),'type'=>$type))));
            }else{
                $_POST['token'] = session('token');
                if(!$t_busines->create()){
                    ...
                    if('edit'==$status && $bid != ''){
                        ...
                        if($o){
                            ...
                            exit($this->success('修改成功',U("$module/$return_func",array('token'=>session('token'),'type'=>$type))));
                        }else{
                            exit($this->error('修改失败',U("$module/$return_func",array('token'=>session('token'),'type'=>$type))));
                        }
                    }else{
                        ...
                        if($id=$t_busines->data($_POST)->add()){
                            ...
                            M('Keyword')->add($data1);
                            $this->success('添加成功',U("$module/$return_func",array('token'=>session('token'),'type'=>$type)));exit;
                        }else{
                    exit($this->error('务器繁忙,添加失败,请稍候再试',U("$module/$return_func",array('token'=>session('token'),'type'=>$type))));
                        }
                    }//edit & add
                }
    ...
    public function busines_del($module,$type,$return_func){
        $type = filter_var($this->_get('type'),FILTER_SANITIZE_STRING);
        $bid  = filter_var($this->_get('bid'),FILTER_VALIDATE_INT);
        $t_busines = M('busines');
        ...
        $result = $t_busines->where($find)->find();
         if($result){
            ...
            $this->success('删除成功',U("$module/$return_func",array('token'=>session('token'),'type'=>$result['type'])));
             exit;
         }else{
         exit($this->error('非法操作,请稍候再试',U("$module/$return_func",array('token'=>session('token'),'type'=>$type))));
         }
    }
    ...
    public function classify_add($module,$type,$return_func){
        $t_busines = M("busines");
        $where = array('token'=>session('token'),'type'=>$type);
    ...
    public function busines_main_del($module,$type,$return_func){
        $type = filter_var($this->_get('type'),FILTER_SANITIZE_STRING);
        $mid  = filter_var($this->_get('mid'),FILTER_VALIDATE_INT);
        ...
        $result = $t_busines_main->where($find)->find();
         if($result){
            ...
            exit($this->success('删除成功',U("$module/$return_func",array('token'=>session('token'),'type'=>$result['type']))));
         }else{
            exit($this->error('非法操作,请稍候再试',U("$module/$return_func",array('token'=>session('token'),'type'=>$type))));
         }
    }

    public function project_item($module,$type){
    ...
    public function project_item_add($module,$type,$return_func){
        $t_busines_main = M("busines_main");
        $where = array('token'=>session('token'),'type'=>$type);
    ...
    public function project_item_del($module,$return_func){
        $type = filter_var($this->_get('type'),FILTER_SANITIZE_STRING);
        $sid  = filter_var($this->_get('sid'),FILTER_VALIDATE_INT);
    ...
    public function poster_add($module,$type,$return_func){
        $t_busines = M("busines");
        $where = array('token'=>session('token'),'type'=>$type);
        $busines_list = $t_busines->where($where)->order('sort desc')->field('bid,mtitle')->select();
        $this->assign('busines_list',$busines_list);
        ...
        if(IS_POST){
            ...
            $check = filter_var_array($_POST,$filters);
            if(!$check){
                exit($this->error('包含特殊字符,请检查后再提交.',U("$module/$return_func",array('token'=>session('token'),'type'=>$type))));
            }else{
                $_POST['token'] = session('token');
                if(!$t_busines_second->create()){
                    exit($this->error($t_busines_second->getError()));
    ...
    public function poster_del($module,$type,$return_func){
        $type = filter_var($this->_get('type'),FILTER_SANITIZE_STRING);
        $pid  = filter_var($this->_get('pid'),FILTER_VALIDATE_INT);
    ...
    public function comments_add($module,$type,$return_func){
        $t_busines = M("busines");
        $where = array('token'=>session('token'),'type'=>$type);
        $busines_list = $t_busines->where($where)->order('sort desc')->field('bid,mtitle')->select();
        $this->assign('busines_list',$busines_list);
        ...
        if(IS_POST){
            ...
            $check = filter_var_array($_POST,$filters);
            if(!$check){
          exit($this->error('表单包含不允许字符.',U("$module/$return_func",array('token'=>session('token'),'type'=>$type))));
            }else{
                $_POST['token'] = session('token');
                if(!$t_busines_comment->create()){
                    ...
                    exit($this->error('服务器繁忙,添加失败,请稍候再试',U("$module/$return_func",array('token'=>session('token'),'type'=>$type))));
                        }
                    }//edit & add
                }
    ...
    public function comments_del($module,$type,$return_func){
        $type = filter_var($this->_get('type'),FILTER_SANITIZE_STRING);
        $cid  = filter_var($this->_get('cid'),FILTER_VALIDATE_INT);
    ...
    public function orders($module,$type,$return_func){
    ...
    public function order_del($module,$type,$return_func){
        ...
        if(!empty($check)){
            $t_reservebook->where(array('id'=>$check['id'],'token'=>session('token'),'type'=>$type))->delete();
                $this->success('删除成功',U("$module/$return_func",array('token'=>session('token'),'type'=>$type)));
                exit;
        }else{
            $this->error('非法操作！',U("$module/$return_func",array('token'=>session('token'),'type'=>$type)));
            exit;
        }
    }
